...
for (const [name, engine] of [['chromium', chromium], ['firefox', firefox]] as const) {
  ...
  try {
    for (const [value, port, keys] of [['warehouse', 927, 1], ['projects', 928, 2]] as const) {
      await ready();
      const count = departures.length;
      const url = page.url();
      const select = page.getByLabel('Module', { exact: true });
      await select.focus();
      for (let i = 0; i < keys; i++) await page.keyboard.press('ArrowDown');
      await page.waitForTimeout(150);
      check(departures.length === count && page.url() === url && await select.inputValue() === value && await select.evaluate(el => el === document.activeElement), 'Module arrows browse without navigation or focus loss', value);
      await page.keyboard.press('Tab');
      check(await page.getByRole('button', { name: 'Open selected module' }).evaluate(el => el === document.activeElement), 'Module Open follows select in tab order', value);
      await page.keyboard.press('Enter');
      await page.waitForTimeout(200);
      check(departures.length === count + 1 && departures.at(-1) === `https://node-ss.tail640a1e.ts.net:${port}/`, 'Explicit Open requests the chosen module only', departures.at(-1));
    }
    await ready();
    ...
    for (const width of [360, 390, 440, 768, 1024, 1280, 1440]) {
      ...
      const fit = await page.evaluate(() => {
        const selectors = ['.ask-launch', '.section-control', '.module-control', '.mast-tools'];
    ...
    check(false, 'Browser checks completed', String(error));
  } finally {
    await browser.close();
  }
}
